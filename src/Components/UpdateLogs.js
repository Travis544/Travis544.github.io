...
const threshHoldDate=new Date(2021, 6, 6, 0, 0, 0, 0);
useEffect(()=>{
    let arr=[]
    fetch(url)
    ...
    arr.push({message:"Finished homepage", date: new Date(2021, 6, 6, 0, 0, 0, 0)})
    ...
    <div>
        ...
        <ul>
            {log.map((commit, index)=>{
                if (commit.date>=threshHoldDate){
                    // {timeStrin(commit.date)}
                    return <li key={index}> {commit.date.toDateString() } : {commit.message} </li>
                }
